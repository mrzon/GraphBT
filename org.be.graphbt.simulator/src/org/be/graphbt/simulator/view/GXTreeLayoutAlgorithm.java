package org.be.graphbt.simulator.view;
...
public class GXTreeLayoutAlgorithm extends AbstractLayoutAlgorithm {
	...
	protected void applyLayoutInternal(InternalNode[] entitiesToLayout, InternalRelationship[] relationshipsToConsider,
		...
		double[] maxHeights = new double[rows.size()];
		// the vertical size of a node line
//		double maxHeight = -1;
		double maxWidth = -1;
		for(int i = 0; i < maxHeights.length;++i) {
			double temp = -1;
			for (GXMoreThanNode item : rows.get(i)) {
				if((temp=item.m_node.getHeightInLayout()) > maxHeights[i])
					maxHeights[i] = temp;
			}
		}
		double temp=-1;
		for (InternalNode list : entitiesToLayout) {
			
//			if((temp=list.getLayoutEntity().getHeightInLayout()) > maxHeight) {
//				maxHeight = temp;
//			}
			if((temp=list.getLayoutEntity().getWidthInLayout()) > maxWidth) {
				maxWidth = temp;
			}
		}
...
//		int verticalLineSize =(int) (maxHeights[] + verSpacing);//(int)(entitiesToLayout[0].getLayoutEntity().getHeightInLayout() + verSpacing);
		
		// the vertical location we begin placing the nodes from
		int sum = 0;
		for(int i = 0;i < maxHeights.length;++i)
			sum += (int) (maxHeights[i] + verSpacing);
		int heightSoFar = sum;//(int) (((rows.size()-1) * verticalLineSize) + verSpacing);
		
		// place bottom most row first - just center it.
		List<GXMoreThanNode> childRow = rows.get(0);
		int nodeSpacing = (int) (horSpacing + maxWidth);//(int) (horSpacing + childRow.get(0).getNode().getLayoutEntity().getWidthInLayout());
		...
		for (int i = 1; i < rows.size(); i++) {
			...
			heightSoFar -= (int)(maxHeights[i] + verSpacing);//verticalLineSize;
	...
	private void placeChildless(List<GXMoreThanNode> childlessStrand,
		...
		int startMark = 0;
		int spacing = (int)(childlessStrand.get(0).getNode().getLayoutEntity().getWidthInLayout() + horSpacing);
	...
	private void shiftTreesRightOfMark(GXMoreThanNode mark, int shift) {
		...
		List<GXMoreThanNode> treeRoots = leftMostChild.getRow().subList(leftMostChild.getRow().indexOf(leftMostChild), leftMostChild.getRow().size());
